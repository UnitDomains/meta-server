...
public class OwnerDomainNameRepositoryImpl implements OwnerDomainNameRepository {
    ...
    private int getRegistrantDomainsCount(int networkId,
                                          String address) {
    ...
    private List<OwnerDomainName> getRegistrantDomainsPageQuery(int networkId,
                                                                String address,
                                                                int pageNo,
                                                                int pageSize) {
    ...
    public Page<OwnerDomainName> getRegistrantDomainsPage(int networkId,
        ...
        long totalCount = getRegistrantDomainsCount(networkId,
                                                    address);
        if (totalCount < 1) return new Page<>();
        int startIndex = Page.getStartOfPage(pageNo,
                                             pageSize);
        List<OwnerDomainName> resultData = getRegistrantDomainsPageQuery(networkId,
                                                                         address,
                                                                         pageNo - 1,
                                                                         pageSize);
        return new Page<>(0,
    ...
    private int getControllerDomainsCount(int networkId,
                                          String address) {
        return jdbcTemplate.queryForObject("""
                                                   SELECT count(*)
    ...
    private List<OwnerDomainName> getControllerDomainsPageQuery(int networkId,
                                                                String address,
                                                                int pageNo,
                                                                int pageSize) {
        return jdbcTemplate.query("""
    ...
    public Page<OwnerDomainName> getControllerDomainsPage(int networkId,
                                                          ...
                                                          int pageNo,
                                                          int pageSize) {
        long totalCount = getControllerDomainsCount(networkId,
                                                    address);
        if (totalCount < 1) return new Page<>();
        int startIndex = Page.getStartOfPage(pageNo,
                                             pageSize);
        List<OwnerDomainName> resultData = getControllerDomainsPageQuery(networkId,
                                                                         address,
                                                                         pageNo - 1,
                                                                         pageSize);
        return new Page<>(0,
    ...
    @Override
    public OwnerDomainName getOwnerDomainNameByLabel(int networkId,
                                                     String label) {
    ...
    private static final class OwnerDomainNameMapper implements RowMapper<OwnerDomainName> {
        @Override
        public OwnerDomainName mapRow(ResultSet rs,
                                      int rowNum) throws SQLException {
            OwnerDomainName ownerDomainName = new OwnerDomainName();
            ownerDomainName.setName(rs.getString("name"));
            ownerDomainName.setBaseNodeIndex(rs.getInt("base_node_index"));
